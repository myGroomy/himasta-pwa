...
import {
  ...
  Bell, Users, Globe, CalendarClock
} from 'lucide-react'
...
export function BottomNav({ user, divisions }: { user: SessionUser; divisions: NavDivision[] }) {
  ...
  const getMainTabs = (): TabItem[] => {
    if (isBPH) {
      return [
        ...
        { href: '/kegiatan', label: 'Kegiatan', icon: CalendarClock, responsive: 'desktop' },
    ...
    if (isKadiv) {
      return [
        ...
        { href: '/proker', label: 'Proker', icon: Target },
        { href: '/admin/approval', label: 'Approval', icon: ShieldCheck },
        { href: '/kegiatan', label: 'Kegiatan', icon: CalendarClock },
        { href: '/announcements', label: 'Pengumuman', icon: Megaphone, responsive: 'desktop' },
        { href: '/dokumen', label: 'Dokumen', icon: FolderOpen, responsive: 'desktop' },
  ...
  const allFeatures = [
    ...
    { href: '/kegiatan', label: 'Kegiatan', icon: CalendarClock, color: 'text-pastel-blue-foreground bg-pastel-blue' },
    { href: '/dokumen', label: 'Dokumen', icon: FolderOpen, color: 'text-foreground bg-secondary border border-border' },
    { href: '/welcome', label: 'Event & Landing', icon: PartyPopper, color: 'text-pastel-yellow-foreground bg-pastel-yellow' },
    { href: '/kalender', label: 'Kalender', icon: CalendarDays, color: 'text-foreground bg-secondary border border-border' },
    { href: '/direktori', label: 'Direktori', icon: Users, color: 'text-pastel-blue-foreground bg-pastel-blue' },
    { href: '/izin', label: 'Izin', icon: FileClock, color: 'text-foreground bg-secondary border border-border' },
    { href: '/search', label: 'Cari', icon: Search, color: 'text-foreground bg-secondary border border-border' },
    { href: '/welcome', label: 'Landing', icon: Globe, color: 'text-foreground bg-secondary border border-border' },
  ]

  const adminFeatures = [
    { href: '/admin/approval', label: 'Approval Center', icon: ShieldCheck, color: 'text-pastel-green-foreground bg-pastel-green' },
    { href: '/events', label: 'Kelola Event', icon: PartyPopper, color: 'text-pastel-yellow-foreground bg-pastel-yellow' },
    ...
    { href: '/admin/periode', label: 'Manajemen Periode', icon: History, color: 'text-foreground bg-secondary border border-border', bphOnly: true },
  ]
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                {allFeatures.map(f => {
                  ...
                  return (
                    <Link key={f.href} href={f.href} onClick={() => setShowMoreSheet(false)} className={`flex flex-col items-center justify-center gap-1.5 rounded-xl border border-border bg-background p-3 text-center text-xs font-medium hover:bg-secondary transition-colors${hiddenClass}`}>
