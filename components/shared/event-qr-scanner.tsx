...
const SCANNER_ID = 'event-qr-reader-region'
...
export function EventQrScanner() {
  ...
  async function submitToken(token: string) {
    ...
    try {
      ...
      if (res.ok) {
        setStatus('success')
        setMessage(`Kehadiran tercatat untuk ${data?.registration?.name ?? 'peserta'}.`)
      } else {
        setStatus('error')
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        setStatus('scanning')
        setMessage('Arahkan kamera ke QR peserta event')
        ...
        setMessage('Kamera tidak dapat diakses langsung. Ambil foto QR peserta.')
        console.error(err)
      }
    }
